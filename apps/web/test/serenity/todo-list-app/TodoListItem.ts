...
export class TodoListItem {
  ...
  static markAsCompleted = (item: QuestionAdapter<PageElement>) =>
    Task.where(
      ...
      Check.whether(CssClasses.of(item), not(contain("completed"))).andIfSo(this.toggle(item)),
    );

  static markAsOutstanding = (item: QuestionAdapter<PageElement>) =>
    Task.where(
      d`#actor marks ${item} as outstanding`,
      Check.whether(CssClasses.of(item), contain("completed")).andIfSo(this.toggle(item)),
    );

  static toggle = (item: QuestionAdapter<PageElement>) =>
    Task.where(
      d`#actor toggles the completion status of ${item}`,
      Click.on(this.#toggleButton().of(item)),
    );
